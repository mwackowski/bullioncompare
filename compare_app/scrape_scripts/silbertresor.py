...
#         ['1/4 oz', [0.25,'']],
    ...
    ['inne',[0, 'https://www.silbertresor.de/index.php?cat=c63_Kleine-Silbermuenzen.html']],
...
df['PRICE_PLN']= df['PRICE'].round(2).apply(lambda x: '%.2f' % x).replace('inf','-').astype(str).apply(lambda x: x+' zł' if x!='-' else x).apply(lambda x: x.replace('.', ','))
...
with engine.connect().execution_options(autocommit=True) as conn:
    conn.execute(text("DELETE from compare_app_pricings where SHOP = 'Silbertresor'"))
